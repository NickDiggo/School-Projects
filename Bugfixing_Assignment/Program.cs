...
// aankoop.Pc = new Pc(); // Voeg dit toe om Pc correct te initialiseren
...
while (!aankoop.Pc.PcIsCompleet())
{
    ...
    else
    {
        aankoop.Pc = new Pc(
            aankoop.Pc.Geheugen,
            aankoop.Pc.Moederbord,
            aankoop.Pc.Processor,
            aankoop.Pc.GrafischeKaart
        );
    }
}
...
{ //titel naar moederbord
    ToonKeuzeTitel("Moederbord");
    //filterproccesoren naar moederborden
...
string KeuzeOptie(string optie)
{
    ...
    do
    {
        ...
        string.IsNullOrWhiteSpace(invoer) || (invoer.ToLower() != "ja" && invoer.ToLower() != "nee")
    );
    return invoer;
}
...
Software KiesSoftware()
{
    ...
    //softwarelijst zonder +1 in return
