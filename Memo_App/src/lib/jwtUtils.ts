...
export const createJwtToken = (user: Profile) => {
  return JWT.sign(
    ...
    PRIVATE_KEY_DECODED,
    {algorithm: 'RS256', expiresIn: TOKEN_EXPIRATION, subject: user.email, issuer: 'memos-app'},
  )
}
...
export const createStatefulJwtToken = (session: SessionWithProfile) => {
  return JWT.sign(
    ...
    PRIVATE_KEY_DECODED,
    {algorithm: 'RS256', expiresIn: TOKEN_EXPIRATION, subject: session.user.email, issuer: 'memos-app'},
  )
}
